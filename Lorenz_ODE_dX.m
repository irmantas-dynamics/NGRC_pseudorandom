...
% Map: X_{n+1} = X_n + W_{out} * P(X_n,X_{n-1},...)  
%
close all
clear all
...
% derivative approximation from signal
dXnorm = (Xnorm(2:end)-Xnorm(1:end-1))/dt;
...
title('Training signal (derivative)- one step prediction')
...
dX_valid_norm = (Xvalidnorm(2:end) - Xvalidnorm(1:end-1))/dt;

Pvalid = feature_matrix(Xvalidnorm, H, M, pairs,0.01);
Uvalid = dX_valid_norm(H:end);
...
legend({'True','Predic'})
xlabel('t')
title('Validation signal (derivative) - one step prediction')
subplot(212)
...
X_long_pred = NaN(validation_points-H,1);
for t1 = 1:validation_points-H-1
    ...
    X_long_pred(t1) = U_long_pred(t1)*dt+X(end);
    X = [X(2:end) X_long_pred(t1)];   
end

figure
subplot(211)
plot( (Tvalid-Tvalid(H))/Tlyap,Xvalidnorm), hold on
plot( (Tvalid(H+1:end)-Tvalid(H))/Tlyap,X_long_pred,'--')
legend({'True','Pred'})
xlim([-Tvalid(H)/Tlyap inf])
title('Long time prediction')
xlabel('$\Lambda t$','Interpreter','latex')
subplot(212)
plot( (Tvalid(H+1:end)-Tvalid(H))/Tlyap ,Xvalidnorm(H+1:end)-X_long_pred), hold on
legend({'True-Pred'})
xlabel('$\Lambda t$','Interpreter','latex')
xlim([0,3])
ylim([-0.025 0.025])
